...
DECLARE
  new_user_id uuid;
BEGIN
  -- Seed test user for authenticated checkout
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'brsolutiontransport@gmail.com') THEN
    ...
    VALUES (new_user_id, 'BR Solution Transport')
    ON CONFLICT (id) DO NOTHING;
  END IF;
END $$;

-- Ensure RLS allows public/authenticated reading of products and related tables
